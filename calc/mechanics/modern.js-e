...
function calculateModern(gen, attacker, defender, move, field) {
    ...
    if (defender.hasItem('Ring Target') && typeEffectiveness === 0) {
        var damageTaken = gen.types.get(util_1.toID(move.type)).damageTaken;
    ...
    if (field.weather === 'Strong Winds' &&
        ...
        gen.types.get(util_1.toID(move.type)).damageTaken['Flying'] > 1) {
        typeEffectiveness /= 2;
        description.weather = field.weather;
    }
    if (move.type === 'Ground' &&
    ...
    basePower = Math.max(1, util_2.pokeRound((basePower * chainMods(bpMods)) / 0x1000));
    var attack;
    ...
    attack = Math.max(1, util_2.pokeRound((attack * chainMods(atMods)) / 0x1000));
    var defense;
    ...
    defense = Math.max(1, util_2.pokeRound((defense * chainMods(dfMods)) / 0x1000));
    ...
    if (util_2.isGrounded(attacker, field)) {
        if (field.terrain === 'Electric' && move.type === 'Electric') {
            baseDamage = util_2.pokeRound((baseDamage * 0x14cd) / 0x1000);
            description.terrain = field.terrain;
        }
    ...
    for (var i = 0; i < 16; i++) {
        ...
        if (attacker.ability === 'Parental Bond' &&
            ...
            var bondFactor = gen.num < 7 ? 3 / 2 : 5 / 4;
...
exports.calculateModern = calculateModern;
...
function getFinalDamage(baseAmount, i, effectiveness, isBurned, stabMod, finalMod) {
    var damageAmount = Math.floor(util_2.pokeRound((Math.floor((baseAmount * (85 + i)) / 100) * stabMod) / 0x1000) * effectiveness);
    if (isBurned) {
        damageAmount = Math.floor(damageAmount / 2);
    }
    return util_2.pokeRound(Math.max(1, (damageAmount * finalMod) / 0x1000));
}
