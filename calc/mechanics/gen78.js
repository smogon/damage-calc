...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    if (attacker.hasAbility('Steely Spirit') && move.type === 'Steel') {
        bpMods.push(0x1800);
        description.attackerAbility = attacker.ability;
    }
    ...
    if (move.name === 'Solar Beam' && field.hasWeather('Rain', 'Heavy Rain', 'Sand', 'Hail')) {
        bpMods.push(0x800);
    ...
    if ((attacker.hasAbility('Slow Start') &&
        ...
        (attacker.named('Cherrim') &&
            ...
            field.hasWeather('Sun', 'Harsh Sunshine') &&
            move.category === 'Physical') ||
        (attacker.hasAbility('Gorilla Tactics') && ['Gmax', 'Dynamax'].indexOf(attacker.name) === -1)) {
        atMods.push(0x1800);
        description.attackerAbility = attacker.ability;
        description.weather = field.weather;
    }
    ...
    else if (attacker.hasAbility('Steelworker') && move.type === 'Steel') {
        atMods.push(0x1800);
        description.attackerAbility = attacker.ability;
    }
    ...
    else if ((attacker.hasAbility('Water Bubble') && move.type === 'Water') ||
    ...
    if ((defender.hasItem('Eviolite') && ((_a = gen.species.get(util_1.toID(defender.name))) === null || _a === void 0 ? void 0 : _a.canEvolve)) ||
    ...
    if (defender.isDynamaxed &&
        ['Dynamax Cannon', 'Behemoth Blade', 'Behemoth Bash'].includes(move.name)) {
        finalMods.push(0x2000);
    }
    ...
    else if ((defender.hasAbility('Punk Rock') && move.isSound) ||
    ...
    for (var i = 0; i < 16; i++) {
        damage[i] = util_2.getFinalDamage(baseDamage, i, typeEffectiveness, applyBurn, stabMod, finalMod, protect);
