...
export function calculateSMSSSV(
  ...
  (attacker.hasAbility('Unseen Fist') && move.flags.contact) || attacker.hasItem('Sledgehammer');
  ...
  if (attacker.hasItem('Nurse Hat') && move.bp <= 50 && typeEffectiveness <= 0.5 && typeEffectiveness > 0) {
    typeEffectiveness = 0;
    desc.attackerItem = attacker.item;
    desc.moveName = 'Heal Pulse';
    return result;
  }
  ...
  if (fixedDamage) {
    if (attacker.hasAbility('Parental Bond') || (attacker.hasItem('Queen\'s Gland') && attacker.hasType('Bug') && attacker.gender === 'F')) {
  ...
  if (field.defenderSide.isProtected &&
    (attacker.isDynamaxed || (move.isZ && attacker.item && attacker.item.includes(' Z')) || (attacker.hasItem('Sledgehammer') && !move.named('Fling')))) {
    protect = true;
    desc.isProtected = true;
  }
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasType('Bug') && attacker.hasItem('Queen\'s Gland') && attacker.gender === 'F' && move.hits === 1 && !isSpread) {
    const child = attacker.clone();
    child.ability = 'Parental Bond (Child)' as AbilityName;
...
export function calculateFinalModsSMSSSV(
  ...
  if (attacker.hasItem('Expert Belt') && typeEffectiveness > 1 && !move.isZ) {
    ...
    finalMods.push(5324);
    desc.attackerItem = attacker.item;
  } else if (attacker.hasItem('Sharpshooter\'s Lens') && isCritical) {
    finalMods.push(6144);
    desc.attackerItem = attacker.item;
