...
function calculate() {
    ...
    for (var i = 0; i < 4; i++) {
		...
		if (p1.moves[i].givesHealth) {
            ...
            additionalText = ' (recovers between ' + minHealthRecovered + notation + ' and ' + maxHealthRecovered + notation + ')';
		}
		
        $(resultLocations[0][i].move + " + label").text(p1.moves[i].name.replace("Hidden Power", "HP"));
        $(resultLocations[0][i].damage).text(minDisplay + " - " + maxDisplay + notation + additionalText);
        ...
        $(resultLocations[1][i].damage).text(minDisplay + " - " + maxDisplay + notation + additionalText);
        if (maxDamage > highestDamage) {
            highestDamage = maxDamage;
